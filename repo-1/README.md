...
גרסת הדגמה חיה, ללא התקנה: **https://hamoudehh.github.io/claude-code-learn/**

היא נבנית אוטומטית מ-`main` בכל דחיפה. בגרסה הזו אין שרת — הנתונים נשמרים בדפדפן של כל מכשיר בנפרד ואינם משותפים. להזנה אמיתית ומשותפת יש להריץ מקומית לפי ההוראות שלהלן.

## דרישה מוקדמת

יש להתקין **Node.js 22.5 ומעלה** (מומלץ להתקין את גרסת ה-LTS, 24): https://nodejs.org
...
- מסד הנתונים הוא `node:sqlite` — מנוע ה-SQLite המובנה ב-Node, ולכן אין תלות native ואין צורך בכלי קומפילציה.
- ניצולת זמן יכולה לעבור 100% אם נרשמת יותר ממשמרת אחת ביום לאותה מכונה.
